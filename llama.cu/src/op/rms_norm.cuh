...
template <unsigned int BLOCK_SIZE, class Tw, class Ta>
static __device__ void padding_2d(
    Ta *__restrict__ y_,
    int const stride_y,
    ...
    padding<BLOCK_SIZE>(y, x, w_, epsilon);
}
...
template <unsigned int BLOCK_SIZE, unsigned int NUM_ITEMS_THREAD, class Tw, class Ta>
static __device__ void folding_2d(
    Ta *__restrict__ y,
    int const stride_y,
    Ta const *__restrict__ x,
    int const stride_x,
    ...
    folding<BLOCK_SIZE, NUM_ITEMS_THREAD>(y, x, w, epsilon, items_size);
}
